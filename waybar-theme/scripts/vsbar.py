...
def _cpu():
        ...
        with open("/proc/stat") as f:
            for line in f:
                if line.startswith("cpu"):
                    ...
                    # idle = idle + iowait
                    stats[name] = (vals[3] + vals[4], sum(vals))
        return stats
    ...
    for cpu_name in s1:
        ...
        i2, t2 = s2[cpu_name]
        dt = t2 - t1
        # Calculate percentage; 100 - (idle_diff / total_diff)
        pct = int(100 - (i2 - i1) * 100 / dt) if dt else 0
        percentages[cpu_name] = pct
...
if args.type == "cpu":
    ...
    # Use the aggregate 'cpu' key for the main bar/label
    total_pct = all_pcts.pop("cpu") 
    
    # Build the tooltip string for individual cores
    # Sort keys to ensure CPU0, CPU1, CPU2... order
    core_details = []
    for cpu_core in sorted(all_pcts.keys(), key=lambda x: int(x[3:])):
        core_details.append(f"{cpu_core.upper()}: {all_pcts[cpu_core]}%")
    
    tooltip_text = "\n".join(core_details)
    
    bar = make_bar(total_pct, W, F, E)
    lbl = "" if args.no_label else f" {total_pct}%"
